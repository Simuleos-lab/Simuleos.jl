...
- Scope creation currently handled by `@sim_rawscope` (related to plumbing).
- Scope may remain a plain Dict (reason: simplicity, transparency).
- Scope should not be mutated by hooks (constraint).

---

## Labels
- Labels created by `@sim_label`.
- Label keys follow `_sim_label_HASH`.
