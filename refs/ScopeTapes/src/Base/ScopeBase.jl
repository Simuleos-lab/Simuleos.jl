...
Base.setindex!(s::Scope, val::ScopeVariable, key::String) = (s.sc[key] = val)
...
function Base.show(io::IO, sc::Scope)
    print(io, "🌐 Scope [")
    printstyled(io, sc.sc_hash; color = :blue)
    println(io, "] with $(length(sc.sc)) variable(s):\n")

    # format tape variables
    key_map = map(collect(keys(sc))) do key
        st_is_labelkey(key) && return (key, "st_label")
        return (key, key)
    end
