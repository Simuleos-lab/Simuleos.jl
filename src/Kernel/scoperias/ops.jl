...
function filter_vars(f::Function, scope::SimuleosScope)::SimuleosScope
    out = Dict{Symbol, ScopeVariable}()
    for (k, v) in scope.variables
        f(k, v) || continue
        out[k] = v
    end
    return SimuleosScope(copy(scope.labels), out, copy(scope.metadata))
end

function filter_vars!(f::Function, scope::SimuleosScope)::SimuleosScope
    for k in collect(keys(scope.variables))
        f(k, scope.variables[k]) || delete!(scope.variables, k)
    end
    return scope
end

function filter_labels(f::Function, scope::SimuleosScope)::SimuleosScope
    return SimuleosScope([l for l in scope.labels if f(l)], copy(scope.variables), copy(scope.metadata))
end
...
function merge_scopes(scopes::SimuleosScope...)::SimuleosScope
    ...
    for s in scopes
        for l in s.labels
            l in out.labels || push!(out.labels, l)
        end
        for (k, v) in s.variables
            out.variables[k] = v
        end
        merge!(out.metadata, s.metadata)
    end
    return out
end
