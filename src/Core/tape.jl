# JSONL tape I/O for recording simulation sessions

function _append_to_tape(recorder::Core.SessionRecorder, root_dir::String, commit_label::String="")
    # Use session-specific directory
    safe_label = replace(recorder.label, r"[^\w\-]" => "_")
    session_dir = joinpath(root_dir, "sessions", safe_label)
    tapes_dir = joinpath(session_dir, "tapes")
    mkpath(tapes_dir)

    tape_path = joinpath(tapes_dir, "context.tape.jsonl")

    open(tape_path, "a") do io
        Core._write_commit_record(io, recorder, root_dir, commit_label)
        println(io)
    end
end
