...
@testset "Simignore" begin
    @testset "Rule validation" begin
        ...
        @test_throws ErrorException Simuleos.ContextIO.set_simignore_rules!(session, [
            Dict(:action => :exclude)
        ])
        ...
        Simuleos.ContextIO.set_simignore_rules!(session, [
    ...
    @testset "_should_ignore - type filtering" begin
        ...
        @test Simuleos.ContextIO._should_ignore(session, :Base, Base, "scope1") == true

        # Functions are always ignored
        @test Simuleos.ContextIO._should_ignore(session, :println, println, "scope1") == true

        # Regular values are not ignored by default (no rules)
        @test Simuleos.ContextIO._should_ignore(session, :x, 42, "scope1") == false
        @test Simuleos.ContextIO._should_ignore(session, :data, [1, 2, 3], "scope1") == false
    end
    ...
    @testset "_should_ignore - global rules" begin
        ...
        @test Simuleos.ContextIO._should_ignore(session, :_temp, 1, "any_scope") == true

        # result should not be ignored
        @test Simuleos.ContextIO._should_ignore(session, :result, 1, "any_scope") == false

        # _private should be ignored
        @test Simuleos.ContextIO._should_ignore(session, :_private, "secret", "other_scope") == true
    end
    ...
    @testset "_should_ignore - scope-specific rules" begin
        ...
        @test Simuleos.ContextIO._should_ignore(session, :debug_info, 1, "production") == true

        # debug_info should NOT be ignored in "development" scope
        @test Simuleos.ContextIO._should_ignore(session, :debug_info, 1, "development") == false

        # other vars not affected
        @test Simuleos.ContextIO._should_ignore(session, :result, 1, "production") == false
    end
    ...
    @testset "_should_ignore - last rule wins" begin
        ...
        @test Simuleos.ContextIO._should_ignore(session, :_temp, 1, "scope") == true

        # _temp_data should be excluded (only first rule matches)
        @test Simuleos.ContextIO._should_ignore(session, :_temp_data, 1, "scope") == true

        # _temp_keep should be included (second rule wins)
        @test Simuleos.ContextIO._should_ignore(session, :_temp_keep, 1, "scope") == false

        # _temp_keep_extra should be included (both rules match, second wins)
        @test Simuleos.ContextIO._should_ignore(session, :_temp_keep_extra, 1, "scope") == false
    end
    ...
    @testset "_should_ignore - mixed global and scope rules" begin
        ...
        @test Simuleos.ContextIO._should_ignore(session, :_temp, 1, "dev") == true
        @test Simuleos.ContextIO._should_ignore(session, :_temp, 1, "prod") == true

        # _debug included in dev (scope rule wins)
        @test Simuleos.ContextIO._should_ignore(session, :_debug, 1, "dev") == false

        # _debug excluded in prod (only global rule matches)
        @test Simuleos.ContextIO._should_ignore(session, :_debug, 1, "prod") == true
    end
end
